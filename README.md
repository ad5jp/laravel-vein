...
Entry 等から見て 1:1 (BelongsTo) で紐づく。  
...
## データベース設計のルール  
### ファイルアップロード (File)  
アップロードファイルは、ファイルの種類ごとに独立したモデルとして扱う必要がある。  
主データ (Entry, Taxonomy, Record, Page) から見て **BelongsTo リレーション**として  
定義する。  
(HasOne リレーションには対応していない)  

**1:N には未対応。** FileUpload は BelongsTo 専用で、FileUploadMultiple は未実装。  
複数のファイルを持たせたい場合は、Record のサブテーブルを作り、その各行に  
1 つずつファイルをぶら下げる。  
